...
export const CardFeeContent = ({ title, givenIcon: GivenIcon, feeValues, release, size }: Readonly<CardFeeType>) => (
    <CardContent className={`d-flex flex-row row align-items-start px-5 col-${size ?? 4}`}>
...
function BestFee({ children }: Readonly<CardFeeContainerType>) {
    return (
        <Card className={`d-flex flex-row py-5 w-100 ` + styles.background_card} variant="outlined" >
            {children}
        </Card >
    )
}
